...
// operations
template<size_t D, typename T>
T operator*(const Vector<D, T>& v1, const Vector<D, T>& v2) {
	T ret = T();
...
Vector<D, T> operator*(Vector<D, T> v1, const U& v2) {
...
template<size_t D, typename T>
bool operator==(const Vector<D, T>& v1, const Vector<D, T>& v2) {
...
Vector<L, T> homorize(const Vector<D, T>& v, T fill = 1) {
	Vector<L, T> ret;
...
template<size_t L, size_t D, typename T>
Vector<L, T> proj(const Vector<D, T>& v) {
	Vector<L, T> ret;
...
template<size_t D, typename T>
std::ostream& operator<<(std::ostream& os, Vector<D, T>& v) {
	for (int i = 0; i < D; i++) {
		os << v[i] << " ";
	}
	return os;
...
template<>
struct CrossVecImplement <vector3i, 3, 3> {
	static inline void eval(vector3i&, const vector3i&, const vector3i&) {}
};
...
template<>
vector3i cross(const vector3i& v1, const vector3i& v2) {
	vector3i ret;
	CrossVec(vector3i, ret, v1, v2, 3);
	return ret;
}
...
public:
	...
	inline Vector<R, T> col(const size_t idx) const {
		assert(idx < C);
		Vector<R, T> ret;
		for (size_t i = R; i--; ret[i] = row[i][idx]);
		return ret;
	}

	inline void set_col(size_t idx, Vector<R, T> v) {
		assert(idx < C);
		for (size_t i = R; i--; row[i][idx] = v[i]);
	}
};
...
template<size_t R, size_t C, typename T>
Matrix<R, C, T> operator/(Matrix<R, C, T> mat, const T& f) {
